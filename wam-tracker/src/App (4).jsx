...
// ── FIX: Local-state text inputs to prevent cursor/glitch on every keystroke ──
function GoalTitleInput({ value, onChange, inputBase }) {
  const [local, setLocal] = useState(value);
  // Keep local in sync when parent resets (e.g. switching edit mode off)
...
function TacticTextInput({ value, onChange, inputBase }) {
  const [local, setLocal] = useState(value);
  const prevValue = useRef(value);
  useEffect(() => {
    if (prevValue.current !== value && local !== value) {
...
function MemberNameInput({ value, onChange, memberColor, t }) {
  const [local, setLocal] = useState(value);
  const prevValue = useRef(value);
  useEffect(() => {
    if (prevValue.current !== value && local !== value) {
  ...
  return (
    <input
      ...
      onChange={e => { setLocal(e.target.value); onChange(e.target.value); }}
      style={{
        background: "transparent", border: "none", borderBottom: `1px solid ${memberColor}`,
...
function SettingsMemberNameInput({ value, onChange, t }) {
  const [local, setLocal] = useState(value);
  const prevValue = useRef(value);
  useEffect(() => {
    if (prevValue.current !== value && local !== value) {
    ...
    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${member.color}30`, overflow: "hidden", boxShadow: t === LIGHT ? "0 1px 4px rgba(0,0,0,0.06)" : "none" }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {isEditing ? (
            <MemberNameInput value={member.name} onChange={onNameChange} memberColor={member.color} t={t} />
          ) : (
            ...
            <div key={goal.id} style={{
              ...
              {isEditing ? (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <GoalTitleInput
                    value={goal.title}
                    onChange={val => updateGoalField(goal.id, "title", val)}
                    inputBase={inputBase}
                  />
                </div>
              ) : goal.title ? (
                  ...
                  return (
                    <div key={tactic.id} style={{
                      ...
                      {isEditing ? (
                        <TacticTextInput
                          value={tactic.text}
                          onChange={val => updateTactic(goal.id, tactic.id, { ...tactic, text: val })}
                          inputBase={inputBase}
                        />
                      ) : tactic.text ? (
...
export default function App() {
  ...
  useEffect(() => {
    // FIX: Only update state from realtime if we are NOT currently editing that member,
    // to prevent Supabase realtime updates from overwriting what the user is typing.
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "members" }, payload => {
        if (payload.eventType === "UPDATE") {
          ...
          setMemberDataMap(prev => {
            // Don't overwrite data for the member currently being edited
            if (editingId === m.id) return prev;
            return { ...prev, [m.id]: m.tracker_data };
          });
        }
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 10 }}>
                  {members.map(m => (
                    <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      />
                      <SettingsMemberNameInput value={m.name} onChange={name => updateMemberName(m.id, name)} t={t} />
                      {members.length > 1 && (
