...
// ── MemberCard — fully self-contained local state while editing ───────────────
function MemberCard({ member, memberData, currentWeek, isEditing, onDoneEditing, onSave, onNameChange, t }) {
  // Keep a fully local copy of the data while editing — no external state changes allowed in
  const [localData, setLocalData] = useState(memberData);
  const [localName, setLocalName] = useState(member.name);

  // Sync in new data from realtime ONLY when not editing
  useEffect(() => {
    if (!isEditing) {
      setLocalData(memberData);
      setLocalName(member.name);
    }
  }, [memberData, member.name, isEditing]);
  ...
  const updateGoalTitle = (gid, title) =>
    setLocalData(d => ({ ...d, goals: d.goals.map(g => g.id === gid ? { ...g, title } : g) }));
  const addTactic = gid =>
  ...
  const updateTacticText = (gid, tid, text) =>
    setLocalData(d => ({ ...d, goals: d.goals.map(g => g.id === gid ? { ...g, tactics: g.tactics.map(t2 => t2.id === tid ? { ...t2, text } : t2) } : g) }));

  // Freq changes: update local + save immediately (no typing involved)
  const updateTacticFreq = (gid, tid, updated) => {
    const newData = { ...localData, goals: localData.goals.map(g => g.id === gid ? { ...g, tactics: g.tactics.map(t2 => t2.id === tid ? updated : t2) } : g) };
    setLocalData(newData);
    onSave(member.id, member.name, member.color, newData);
  };

  // Check toggles: update local + save immediately
  const toggleDay = (gid, tid, week, day) => {
    const newData = {
      ...localData, goals: localData.goals.map(g => g.id === gid ? {
  ...
  // When Done is clicked — save everything at once
  const handleDone = () => {
    onSave(member.id, localName, member.color, localData);
    onNameChange(member.id, localName);
    onDoneEditing();
  };
  ...
  return (
    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${member.color}30`, overflow: "hidden", boxShadow: t === LIGHT ? "0 1px 4px rgba(0,0,0,0.06)" : "none" }}>
                  ...
                  return (
                    <div key={tactic.id} style={{ background: t.surface2, borderRadius: 10, padding: "12px", border: `1px solid ${t.border}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        ...
                        {!isEditing && <span style={{ fontSize: 10, color: t.textGhost, fontFamily: "'DM Mono', monospace" }}>{freqLabel(tactic)}</span>}
                      ...
                      {isEditing ? (
                        <input
                          value={tactic.text}
                          onChange={e => updateTacticText(goal.id, tactic.id, e.target.value)}
                      ...
                      {isEditing && <FreqPicker tactic={tactic} memberColor={member.color} t={t} onChange={updated => updateTacticFreq(goal.id, tactic.id, updated)} />}
      ...
      {/* Done button inside card when editing */}
      {isEditing && (
...
export default function App() {
  ...
  // Real-time: only update cards that are NOT currently being edited
  useEffect(() => {
    const channel = supabase.channel("wam-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "members" }, payload => {
        if (payload.eventType === "UPDATE") {
          const m = payload.new;
          // Don't overwrite if this member is currently being edited
            ...
            {members.map(member => (
              <div key={member.id} style={{ position: "relative" }}>
                <MemberCard
                  ...
                  onSave={(mid, name, color, data) => {
                    setMembers(prev => prev.map(m => m.id === mid ? { ...m, name, color } : m));
                    setMemberDataMap(prev => ({ ...prev, [mid]: data }));
                    saveMemberData(mid, name, color, data);
                  }}
                  onNameChange={(mid, name) => setMembers(prev => prev.map(m => m.id === mid ? { ...m, name } : m))}
                  t={t}
                />
                  ...
                  {members.map(m => (
                    <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <input value={m.name}
                        ...
                        onBlur={e => saveMemberData(m.id, e.target.value, m.color, memberDataMap[m.id] || defaultMemberData())}
                        style={{
