...
const DARK = {
  ...
  btnBg: "#1e1e1e", scrollThumb: "#2e2e2e",
};
const LIGHT = {
  ...
  btnBg: "#efefeb", scrollThumb: "#d0ccc4",
};
...
function ThemeToggle({ dark, onToggle }) {
  return (
    <button onClick={onToggle} style={{
      ...
      borderRadius: 20, cursor: "pointer", padding: "4px 10px", fontSize: 14,
      display: "flex", alignItems: "center", color: dark ? "#888" : "#666",
    }}>{dark ? "☀️" : "🌙"}</button>
  );
}
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          ...
          {DAYS.map((d, i) => (
            <button key={i} onClick={() => {
              const nd = freqDays.map((v, j) => j === i ? !v : v);
              if (nd.some(Boolean)) onChange({ ...tactic, freqDays: nd });
            }} style={{
              width: 26, height: 26, borderRadius: 6, border: "none",
            ...
            return (
              <tr key={wi}>
                <td style={{
                  ...
                  fontFamily: "'DM Mono', monospace", fontWeight: isCurrent ? 700 : 400, fontSize: 10,
                }}>W{wi + 1}</td>
                    ...
                    <td key={di} style={{ textAlign: "center", padding: "1px 2px" }}>
                      <button onClick={() => !disabled && onToggle(wi, di)}
                        ...
                        style={{
                          ...
                          background: !targeted ? "transparent" : isFuture ? t.surface3 : checked ? memberColor : t.surface2,
...
// ── EditForm — completely separate component, mounts once, uses uncontrolled inputs ──
function EditForm({ member, memberData, currentWeek, onSave, onCancel, t }) {
  // Store everything in a ref — never triggers re-render while typing
  const dataRef = useRef(JSON.parse(JSON.stringify(memberData)));
  const nameRef = useRef(null);

  // Scores update only when freq/checks change (buttons), not text
  const [scoreVersion, setScoreVersion] = useState(0);
  const bumpScore = () => setScoreVersion(v => v + 1);

  const goals = dataRef.current.goals || [emptyGoal()];
  const overallScore = calcScore(goals.flatMap(g => g.tactics), currentWeek);

  const addGoal = () => {
    dataRef.current = { ...dataRef.current, goals: [...dataRef.current.goals, emptyGoal()] };
    bumpScore();
  };
  const removeGoal = (gid) => {
    dataRef.current = { ...dataRef.current, goals: dataRef.current.goals.filter(g => g.id !== gid) };
    bumpScore();
  };
  const addTactic = (gid) => {
    dataRef.current = {
      ...dataRef.current,
      goals: dataRef.current.goals.map(g => g.id === gid ? { ...g, tactics: [...g.tactics, emptyTactic()] } : g)
    };
    bumpScore();
  };
  const removeTactic = (gid, tid) => {
    dataRef.current = {
      ...dataRef.current,
      goals: dataRef.current.goals.map(g => g.id === gid ? { ...g, tactics: g.tactics.filter(t2 => t2.id !== tid) } : g)
    };
    bumpScore();
  };
  const updateFreq = (gid, tid, updated) => {
    dataRef.current = {
      ...dataRef.current,
      goals: dataRef.current.goals.map(g => g.id === gid ? {
        ...g, tactics: g.tactics.map(t2 => t2.id === tid ? updated : t2)
      } : g)
    };
    bumpScore();
  };
  ...
  const handleSave = () => {
    // Read all uncontrolled input values at save time
    const name = nameRef.current?.value || member.name;
    const finalData = JSON.parse(JSON.stringify(dataRef.current));
    // Read goal titles from DOM
    finalData.goals.forEach(goal => {
      const el = document.getElementById(`goal-title-${goal.id}`);
      if (el) goal.title = el.value;
      goal.tactics.forEach(tac => {
        const tel = document.getElementById(`tactic-text-${tac.id}`);
        if (tel) tac.text = tel.value;
      });
    });
    onSave(name, finalData);
  };
  ...
  return (
    <div style={{ background: t.surface, borderRadius: 16, border: `2px solid ${member.color}60`, overflow: "hidden" }}>
      {/* Header */}
      <div style={{
        background: member.color + "18", borderBottom: `1px solid ${member.color}25`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <input
            ...
            style={{
              ...
              color: t.text, fontSize: 15, fontWeight: 700, fontFamily: "'Sora', sans-serif", width: 160, outline: "none",
            }}
          />
        </div>
      ...
      <div style={{ padding: "16px" }}>
        {dataRef.current.goals.map((goal, gi) => {
          const gscore = calcScore(goal.tactics, currentWeek);
          return (
            <div key={goal.id} style={{
              marginBottom: 24, paddingBottom: 24,
              borderBottom: gi < dataRef.current.goals.length - 1 ? `1px solid ${t.border}` : "none",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <div style={{
                  background: member.color + "20", color: member.color,
                  borderRadius: 6, padding: "2px 8px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1,
                }}>Goal {gi + 1}</div>
                <ScoreBadge score={gscore} />
                {dataRef.current.goals.length > 1 && (
                  <button onClick={() => removeGoal(goal.id)} style={{ marginLeft: "auto", background: "none", border: "none", color: t.textFaint, cursor: "pointer", fontSize: 16 }}>✕</button>
                )}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {goal.tactics.map(tactic => {
                  ...
                  return (
                    <div key={tactic.id} style={{ background: t.surface2, borderRadius: 10, padding: "12px", border: `1px solid ${t.border}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        ...
                        {goal.tactics.length > 1 && (
                          <button onClick={() => removeTactic(goal.id, tactic.id)} style={{ marginLeft: "auto", background: "none", border: "none", color: t.textFaint, cursor: "pointer", fontSize: 14 }}>✕</button>
                        )}
                      </div>
                      <input
                        id={`tactic-text-${tactic.id}`}
                        defaultValue={tactic.text}
                        placeholder="What habit are you tracking? e.g. Walk 30 min daily"
                        style={{ ...inputBase, marginBottom: 10 }}
                      />
                      <FreqPicker tactic={tactic} memberColor={member.color} t={t}
                        onChange={updated => updateFreq(goal.id, tactic.id, updated)} />
        ...
        <button onClick={handleSave} style={{
          width: "100%", background: "#e8a87c", border: "none", borderRadius: 10,
          color: "#1a1a1a", cursor: "pointer", padding: "10px",
...
// ── Read-only card ────────────────────────────────────────────────────────────
function MemberCard({ member, memberData, currentWeek, t }) {
  ...
  return (
    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${member.color}28`, overflow: "hidden", boxShadow: t === LIGHT ? "0 1px 4px rgba(0,0,0,0.06)" : "none" }}>
      <div style={{
        background: member.color + "14", borderBottom: `1px solid ${member.color}20`,
        padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: member.color, boxShadow: `0 0 6px ${member.color}` }} />
      ...
      <div style={{ padding: "16px" }}>
        {goals.map((goal, gi) => {
          const gscore = calcScore(goal.tactics, currentWeek);
          return (
            <div key={goal.id} style={{ marginBottom: 24, paddingBottom: 24, borderBottom: gi < goals.length - 1 ? `1px solid ${t.border}` : "none" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <div style={{ background: member.color + "20", color: member.color, borderRadius: 6, padding: "2px 8px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Goal {gi + 1}</div>
                <ScoreBadge score={gscore} />
              </div>
              {goal.title ? (
                <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {goal.tactics.map(tactic => {
                  ...
                  return (
                    <div key={tactic.id} style={{ background: t.surface2, borderRadius: 10, padding: "12px", border: `1px solid ${t.border}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        <div style={{ width: 5, height: 5, borderRadius: "50%", flexShrink: 0, background: tscore === null ? t.border3 : tscore >= THRESHOLD ? "#4caf82" : tscore >= 60 ? "#d4a017" : "#d05050" }} />
                        <span style={{ fontSize: 10, color: t.textFaint, textTransform: "uppercase", letterSpacing: 1 }}>I'm Tracking</span>
...
export default function App() {
    ...
    const channel = supabase.channel("wam-realtime")
      .on("postgres_changes", { event: "*", schema: "public", table: "members" }, payload => {
        if (payload.eventType === "UPDATE") {
          const m = payload.new;
          if (m.id === editingId) return; // never interrupt editing
  ...
  const updateMemberColor = (mid, color) => {
    ...
    if (member) saveMemberData(mid, member.name, color, memberDataMap[mid] || defaultMemberData());
  };
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Sora', sans-serif", color: t.text }}>WAM<span style={{ color: "#e8a87c" }}>.</span>tracker</div>
      ...
      <div style={{ padding: "20px", maxWidth: 1200, margin: "0 auto" }}>
              ...
              <div key={member.id} style={{ position: "relative" }}>
                {editingId === member.id ? (
                  <EditForm
                    ...
                    onSave={(name, data) => {
                      ...
                      setMemberDataMap(prev => ({ ...prev, [member.id]: data }));
                ...
                {editingId !== member.id && (
                  <div style={{ position: "absolute", top: 10, right: 12, display: "flex", gap: 6, alignItems: "center" }}>
                    ...
                    <button onClick={() => setEditingId(member.id)} style={{
                      background: t.btnBg, border: "none", borderRadius: 6,
                      color: t.textFaint, cursor: "pointer", fontSize: 11, padding: "3px 10px",
        ...
        {activeTab === "leaderboard" && (
          <div style={{ maxWidth: 480 }}>
            <p style={{ fontSize: 12, color: t.textFaint, marginBottom: 16 }}>Execution scores through Week {currentWeek} · 85%+ is on track</p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 10 }}>
                  {members.map(m => (
                    <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <input type="color" value={m.color} onChange={e => updateMemberColor(m.id, e.target.value)}
                        style={{ width: 34, height: 34, border: "none", borderRadius: 8, cursor: "pointer", background: "none" }} />
                      <input defaultValue={m.name}
                        onBlur={e => {
                          setMembers(prev => prev.map(mb => mb.id === m.id ? { ...mb, name: e.target.value } : mb));
                          saveMemberData(m.id, e.target.value, m.color, memberDataMap[m.id] || defaultMemberData());
                        }}
                        style={{
              ...
              <div>
                ...
                <button onClick={() => setDarkMode(d => !d)} style={{
                  ...
                  fontSize: 13, fontFamily: "'Sora', sans-serif", display: "flex", alignItems: "center", gap: 8,
                }}>{darkMode ? "☀️ Switch to Light Mode" : "🌙 Switch to Dark Mode"}</button>
              </div>
